...
class product_product(osv.osv):
    ...
    _columns = {
        ...
        'pvp_fabricante':fields.float('PVP Fabricante', digits_compute=dp.get_precision('Precio recomendado por fabricante')),
        'pvptt':fields.float('PVT', digits_compute=dp.get_precision('Precio de venta Toptaller')),
        'pvp_taller':fields.float('Precio Venta a Taller', digits_compute=dp.get_precision('Precio de venta al Taller basado en el descuento por defecto')),
        'descuento':fields.float('Descuento por defecto', digits_compute=dp.get_precision('Descuento aplicado por defecto')),
	...
	'pvpms': fields.float('PVP Toptaller', digits_compute=dp.get_precision('Precio establecido basado en el coste')),
        'price2':fields.float('Precio Taller', digits_compute=dp.get_precision('Precio Taller')),
        'standard_price2':fields.float('Precio de Coste', digits_compute=dp.get_precision('Precio de coste del producto')),
        'desctaller':fields.selection(_selection_vals,'Elije descuento', required=False),
        'descfabricante':fields.related('seller_ids', 'supplier_discount', type='float', string='Descuento del proveedor'),
	#'descfabricante':fields.float('Descuento del Fabricante', digits_compute=dp.get_precision('Descuento de Fabricante a MS')),
        ...
        'margen_beneficio':fields.float('Margen beneficio minimo',digits_compute=dp.get_precision('Margen de beneficio que queremos aplicar')),
	...
	'partner_calc_price':fields.float('Precio para cliente',digits_compute=dp.get_precision('Precio para cliente')),
    ...
    def amount(self, cr, uid, ids,partner_calc_price,pricelist_id,partner_id,context=None):
	    ...
	    uom = False
	    qty = 1.0
            price = self.pool.get('product.pricelist').price_get(cr, uid, [pricelist_id],ids[0],qty,partner_id)[pricelist_id] 
	    if price is False:
    ...
    def desctaller_onchange (self, cr, uid, ids,desctaller,pvp,price2,basado_p_coste,standard_price2,descfabricante,margen_beneficio,context=None):
	if context is None:
	    context=context
	...
	### Campos a actualizar de la ficha de producto dirctamente a mysql ###
        #precio_tt = price.pvptt
        #precio_ms = price.list_price2
        #query = '''UPDATE catalog_product_flat_1 SET price= %s, msrp= %s  WHERE entity_id =1''' % (precio_tt,precio_ms)
	
	#try:
        #    conn = mysql.connector.connect(user='importacio',password='123456',host='172.31.45.29',database='importacio') # Conectar a la base de datos 
        ...
        #cursor.execute(query)              # Hacer efectiva la escritura de datos 
        #conn.commit()
